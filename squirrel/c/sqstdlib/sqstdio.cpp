...
public:
	...
	SQFile(SQFILE file, bool owns) { _rhandle = file; _owns = owns;}
	virtual ~SQFile() { Close(); }
	bool Open(const SQChar *filename ,const SQChar *mode) {
		Close();
		if( (_rhandle = sqstd_fopen(filename,mode)) ) {
			_owns = true;
			return true;
		}
		return false;
	...
	void Close() {
		if(_rhandle && _owns) {
			sqstd_fclose(_rhandle);
			_rhandle = NULL;
			_owns = false;
		}
	}
	SQInteger Read(void *buffer,SQInteger size) {
		return sqstd_fread(buffer,1,size,_rhandle);
	}
	SQInteger Write(void *buffer,SQInteger size) {
		return sqstd_fwrite(buffer,1,size,_rhandle);
	}
	SQInteger Flush() {
		return sqstd_fflush(_rhandle);
	}
	SQInteger Tell() {
		return sqstd_ftell(_rhandle);
	}
	SQInteger Len() {
	...
	SQInteger Seek(SQInteger offset, SQInteger origin)	{
		return sqstd_fseek(_rhandle,offset,origin);
	}
	bool IsValid() { return _rhandle?true:false; }
	bool EOS() { return Tell()==Len()?true:false;}
	SQFILE GetHandle() {return _rhandle;}
};
