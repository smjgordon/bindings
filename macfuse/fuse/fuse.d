...
version (DigitalMars) { pragma(lib, "fuse"); }
version (Posix) version (build) pragma (link, "fuse");
 ...
 */
alias int (*fuse_fill_dir_t) (void *buf, /+ const +/ char *name,
				/+ const struct +/ stat_t *stbuf, off_t off);

/* Used by deprecated getdir() method */
struct fuse_dirhandle;
alias /+ struct +/ fuse_dirhandle *fuse_dirh_t;
alias int (*fuse_dirfil_t) (fuse_dirh_t h, /+ const +/ char *name, int type,
			      ino_t ino);
...
struct fuse_operations {
	 ...
	 */
	int (*getattr) (/+ const +/ char *, /* struct */ stat_t *);
	 ...
	 * version 2.5
	 */
	int (*statfs) (/+ const +/ char *, /+ struct +/ statvfs_t *);
...
version (FreeBSD_10) { /+ #if (__FreeBSD__ >= 10) +/
	 ...
	 * Introduced in version 2.5
	 */
	int (*fgetattr) (/+ const +/ char *, /+ struct +/ stat_t *, /+ struct +/ fuse_file_info *);
...
version (FreeBSD_10) { /+ #if (__FreeBSD__ >= 10) +/
	...
	int (*exchange) (/+ const +/ char *, /+ const +/ char *, /+ unsigned long +/ uint);
	...
	int (*setattr_x) (/+ const +/ char *, /+ struct +/ setattr_x_t *);

	int (*fsetattr_x) (/+ const +/ char *, /+ struct +/ setattr_x_t *,
			   /+ struct +/ fuse_file_info *);
 ...
 */
/+ struct +/ fuse_t *fuse_new(/+ struct +/ fuse_chan *ch, /+ struct +/ fuse_args *args,
 ...
 */
void fuse_destroy(/+ struct +/ fuse_t *f);
 ...
 * @param f the FUSE handle
 */
void fuse_exit(/+ struct +/ fuse_t *f);
 ...
 * @return 0 if no error occurred, -1 otherwise
 */
int fuse_loop_mt(/+ struct +/ fuse_t *f);
 ...
 * @return -EINVAL
 */
int fuse_invalidate(/+ struct +/ fuse_t *f, /+ const +/ char *path);
...
int fuse_fs_getattr(/+ struct +/ fuse_fs *fs, /+ const +/ char *path, /+ struct +/ stat_t *buf);
int fuse_fs_fgetattr(/+ struct +/ fuse_fs *fs, /+ const +/ char *path, /+ struct +/ stat_t *buf,
...
int fuse_fs_exchange(/+ struct +/ fuse_fs *fs, /+ const +/ char *oldpath,
		     /+ const +/ char *newpath, /+ unsigned long +/ uint flags);
} /* __FreeBSD__ >= 10 */
...
int fuse_fs_flush(/+ struct +/ fuse_fs *fs, /+ const +/ char *path,
		  /+ struct +/ fuse_file_info *fi);
int fuse_fs_statfs(/+ struct +/ fuse_fs *fs, /+ const +/ char *path, /+ struct +/ statvfs_t *buf);
...
alias void (*fuse_processor_t)(/+ struct +/ fuse_t *, /+ struct +/ fuse_cmd *, void *);

/** This is the part of fuse_main() before the event loop */
/+ struct +/ fuse_t *fuse_setup(int argc, char **argv/+[]+/,
			/+ const struct +/ fuse_operations *op, size_t op_size,
			char **mountpoint, int *multithreaded,
			void *user_data);

/** This is the part of fuse_main() after the event loop */
void fuse_teardown(/+ struct +/ fuse_t *fuse, char *mountpoint);

/** Read a single command.  If none are read, return NULL */
/+ struct +/ fuse_cmd *fuse_read_cmd(/+ struct +/ fuse_t *f);

/** Process a single command */
void fuse_process_cmd(/+ struct +/ fuse_t *f, /+ struct +/ fuse_cmd *cmd);

/** Multi threaded event loop, which calls the custom command
    processor function */
int fuse_loop_mt_proc(/+ struct +/ fuse_t *f, fuse_processor_t proc, void *data);

/** Return the exited flag, which indicates if fuse_exit() has been
    called */
int fuse_exited(/+ struct +/ fuse_t *f);

/** This function is obsolete and implemented as a no-op */
void fuse_set_getcontext_func(/+ struct +/ fuse_context *(*func)(/+void+/));

/** Get session from fuse object */
/+ struct +/ fuse_session *fuse_get_session(/+ struct +/ fuse_t *f);
